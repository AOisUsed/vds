...
type VDRepository interface {
	SetParams(ctx context.Context, id string, params virtualdevice.Params) error // 根据 id 设置虚拟设备状态参数
	Params(ctx context.Context, id string) (virtualdevice.Params, error)         // 根据 id 查找虚拟设备状态参数
	AllParams(ctx context.Context) (map[string]virtualdevice.Params, error)      // 找到所有设备的状态参数
